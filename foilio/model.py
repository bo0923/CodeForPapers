...
#         # 加载并冻结bert模型参数
#         self.bert = RobertaModel.from_pretrained(args.model_path)
...
#  #          param.requires_grad = False
    ...
    def forward(self,premise,prem_mask,adj,conclu,conclu_mask):
        # premise_seq =  premise_seq.to("cuda")


        c_embedded = self.embed(input_ids = conclu,attention_mask = conclu_mask,return_dict = True)["pooler_output"]
       ...
       # print("node_embedding[:,0]",node_embedding[:,0].shape)
        ...
        context_1 = self.attention(c_embedded,node_hidden,node_hidden,prem_mask)  #attn_mask应该是指k-v对中不需要关注的部分
    #    # print(c_cls.shape)
    #    # print(context_1.shape)
    #     context = torch.cat([c_cls,context_1.squeeze(1)], dim=1)
        context = torch.cat([c_embedded,context_1.squeeze(1)],dim = 1)
      
       # n_embedding = n_embedding[0,:]
